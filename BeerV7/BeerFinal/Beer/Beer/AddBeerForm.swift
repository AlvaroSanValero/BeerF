...
struct AddBeerForm: View {
    ...
    @State private var isAddingBeer = false // Estado para controlar la navegación a BeerDetailView
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                // Botones para agregar o cancelar la nueva cerveza
                HStack {
                    // Botón Cancelar
                    Button(action: {
                        dismiss()
                    ...
                    Button(action: {
                        if !name.isEmpty && !beerType.isEmpty && !alcoholContent.isEmpty && !calorieExpense.isEmpty {
                            ...
                            if let image = image {
                                ...
                                let defaultImage = UIImage(named: "defaultImageB")
                            ...
                            if let imageData = imageData {
                                ...
                                if let manufacturer = selectedManufacturer {
                                    ...
                                    dismiss()
                                    isAddingBeer = true // Activa la navegación a BeerDetailView
                                } else {
                    ...
                    .sheet(isPresented: $showingAlert) {
                        CustomAlertView(isPresented: $showingAlert)
                    }
                }
            }
            .padding()
            ...
            // Navegación a BeerDetailView al agregar una nueva cerveza
            .background(
                NavigationLink(destination: BeerDetailView(nameManufacturer: "New Beer"), isActive: $isAddingBeer) {
                    EmptyView()
                }
                .hidden()
            )
